...
Shipped ``turboquant/data/codebooks/*.pt`` files cover common bit-widths so runtime
...
from turboquant.core.types import Codebook
...
def _lloyd_max_gaussian(n_levels: int, max_iter: int = 500, tol: float = 1e-14) -> torch.Tensor:
    ...
    try:
        ...
        raise ImportError(
            ...
            "Install with: pip install turboquant[dev]"
        ) from e
